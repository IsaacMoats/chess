...
public class MoveCalculator {
    ...
    private Collection<ChessMove> promotionMoves(ChessPosition movePosition){
        ArrayList<ChessMove> moves = new ArrayList<>();
        ChessMove forwardLeftMoveQueen = new ChessMove(position, movePosition, ChessPiece.PieceType.QUEEN);
        ChessMove forwardLeftMoveRook = new ChessMove(position, movePosition, ChessPiece.PieceType.ROOK);
        ChessMove forwardLeftMoveBishop = new ChessMove(position, movePosition, ChessPiece.PieceType.BISHOP);
        ChessMove forwardLeftMoveKnight = new ChessMove(position, movePosition, ChessPiece.PieceType.KNIGHT);
        ...
        if (this.color == ChessGame.TeamColor.WHITE) {
                ...
                if (board.getPiece(forwardLeft) != null) {
                    if (board.getPiece(forwardLeft).getTeamColor() == ChessGame.TeamColor.BLACK) {
                        if (forwardLeft.getRow() == 8) {
                            moves.addAll(promotionMoves(forwardLeft));
                        } else {
                ...
                if (board.getPiece(forwardRight) != null) {
                    if (board.getPiece(forwardRight).getTeamColor() == ChessGame.TeamColor.BLACK) {
                        if (forwardRight.getRow() == 8) {
                            moves.addAll(promotionMoves(forwardRight));
                        } else {
            ...
            if (board.getPiece(forwardOne) == null) {
                if (forwardOne.getRow() == 8) {
                    moves.addAll(promotionMoves(forwardOne));
    ...
    private Collection<ChessMove> kingMoveHelper(int rowMove, int colMove) {
        ArrayList<ChessMove> moves = new ArrayList<>();
        ChessPosition nextPosition = new ChessPosition(position.getRow() + rowMove, position.getColumn() + colMove);
        if (board.getPiece(nextPosition) == null) {
            ChessMove forwardMove = new ChessMove(position, nextPosition, null);
    ...
    public Collection<ChessMove> calculateKingMoves(){
        ...
        if (position.getRow() != 8) {
            moves.addAll(kingMoveHelper(1, 0));
            if (position.getColumn() != 1) {
                moves.addAll(kingMoveHelper(1, -1));
            }
            if (position.getColumn() != 8) {
                moves.addAll(kingMoveHelper(1, 1));
            }
        ...
        if (position.getColumn() != 1) {
            moves.addAll(kingMoveHelper(0, -1));
        }
        if (position.getColumn() != 8) {
            moves.addAll(kingMoveHelper(0, 1));
        }
    ...
    private Collection<ChessMove> lineHelper(ChessPosition nextPosition, int rowUpdate, int boardEdge){
        ArrayList<ChessMove> moves = new ArrayList<>();
        while (nextPosition.getRow() != boardEdge) {
            nextPosition =  new ChessPosition(nextPosition.getRow() + rowUpdate, position.getColumn());
            if (board.getPiece(nextPosition) != null) {
                if (board.getPiece(nextPosition).getTeamColor() != color) {
                    ChessMove forwardMove = new ChessMove(position, nextPosition, null);
                    moves.add(forwardMove);
    ...
    private Collection<ChessMove> calculateForwardContinuous(){
        ...
        ChessPosition forward = new ChessPosition(position.getRow(), position.getColumn());
        moves.addAll(lineHelper(forward, 1, 8));
        return moves;
    }
    ...
    private Collection<ChessMove> calculateBackwardContinuous(){
        ...
        ChessPosition backward = new ChessPosition(position.getRow(), position.getColumn());
        moves.addAll(lineHelper(backward, -1, 1));
        return moves;
    }

    private Collection<ChessMove> sideHelper(ChessPosition nextPosition, int columnUpdate, int boardEdge){
        ArrayList<ChessMove> moves = new ArrayList<>();
        while (nextPosition.getColumn() != boardEdge) {
            nextPosition =  new ChessPosition(nextPosition.getRow(), nextPosition.getColumn() + columnUpdate);
            if (board.getPiece(nextPosition) != null) {
                if (board.getPiece(nextPosition).getTeamColor() != color) {
                    ChessMove forwardMove = new ChessMove(position, nextPosition, null);
                    moves.add(forwardMove);
    ...
    private Collection<ChessMove> calculateLeftContinuous(){
        ...
        ChessPosition left = new ChessPosition(position.getRow(), position.getColumn());
        moves.addAll(sideHelper(left, -1, 1));
        return moves;
    }
    ...
    private Collection<ChessMove> calculateRightContinuous(){
        ...
        ChessPosition right = new ChessPosition(position.getRow(), position.getColumn());
        moves.addAll(sideHelper(right, 1, 8));
        return moves;
    }
    ...
    private Collection<ChessMove> calculateKnightMoveHelper(int rowMove, int columnMove){
        ArrayList<ChessMove> moves = new ArrayList<>();
        ChessPosition newPosition = new ChessPosition(position.getRow() + rowMove, position.getColumn() + columnMove);
        ChessMove movement = new ChessMove(position, newPosition, null);
    ...
    public Collection<ChessMove> calculateKnightMoves(){
        ...
        if (position.getRow() <= 6) {
            if (position.getColumn() >= 2) {
                moves.addAll(calculateKnightMoveHelper(2, -1));
            }
        ...
        if (position.getColumn() <= 6) {
            if (position.getRow() >= 2) {
                moves.addAll(calculateKnightMoveHelper(-1, 2));
            }
